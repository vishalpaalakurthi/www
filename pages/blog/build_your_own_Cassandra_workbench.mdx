...
export const meta = {
  ...
  description: 'Skipping logging into terminal for a simple queries',
  date: 'Aug 11, 2020',
  readTime: 2
}
...
Unexpectedly above quotation saved lots of my time, increased my productivity and switched me to create this workbench.
...
i as a developer using Cassandra, most of times use to logging into the terminal and executes the query. it's always kills my productivity because i have to login to terminal for executing simple select queries also.

Most Non-database people like me will only use database for verifying the data. i.e executes select queries only.

Then, i have decided to have my self a simple workbench where i can configure the multiple node sessions and verify the required node data.
